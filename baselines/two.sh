...
do
   ...
   do
       IndriRunQuery -index=$base/indexes/biocaddie_all/ -trecFormat=true -rule=method:two,mu:$mu,lambda:$lambda queries/queries.$col.$topics > output/two/$col/$topics/mu=$mu:lambda=$lambda.out
   done
done
