...
mkdir -p $base/queries/pubmed-rm3/2006
mu=2500
...
do
   ...
   do
      ...
      do
         echo "scripts/run.sh edu.gslis.biocaddie.util.GetFeedbackQueries -input $base/queries/queries.combined.orig.2006 -output $base/queries/pubmed-rm3/2006/queries.mu:$mu,fbTerms:$fbTerms,fbDocs:$fbDocs,fbOrigWeight:$fbOrigWeight -index /data/pubmed/indexes/pubmed/ -fbDocs $fbDocs -fbTerms $fbTerms -rmLambda $fbOrigWeight -maxResults $fbDocs -stoplist data/stoplist.all -mu $mu"
      done
   done
done
